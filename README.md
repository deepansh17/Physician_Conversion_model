...
```bash
conda create -n physician_conversion_model python=3.9
conda activate physician_conversion_model
```
...
```
This command will create your personal repository under `/Repos/<username>/physician_conversion_model`.
...
- To trigger the release pipeline, get the current version from the `physician_conversion_model/__init__.py` file and tag the current code version:
```
